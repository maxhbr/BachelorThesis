...
0 = \Big(\partial_t^2 - \big(t^{-1} + 3\psi'(t)\big)\partial_t
    ...
    + 2 \psi'(t)^2\Big) e\otimes 1
\]
\begin{try}[wie \cite{sabbah_cimpa90}]
...
\end{figure}
\end{try}
\begin{try}[wie \cite{sabbah_Fourier-local}]
und somit mit $\psi(t)=\frac{\lambda}{t^{q}}$ gilt:\\
der neue Operator wird erhalten durch ersetzen von $t^{q+1}\partial_t$ mit
$t^{q+1}-q\lambda$
