...
Familie von $(F_nA)_{n\in\Z}$ von Unterobjekten (Unterring), so dass 
\[ 0\subset\cdots\subset F_n\subset F_{n+1} \subset \cdots \subset A \]
und definiere weiter $gr_k^FA:=F_kA\slash F_{k-1}A$.
\begin{comment}
$gr_k^F$ als was??
\end{comment}
\end{defn}
