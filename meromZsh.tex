...
Ein $P=\sum a_i(x)(x\partial_x)^i$ mit $a_i\in\Cfx$ ist regulär, genau dann
wenn der Koeffizient $a_d$ vor dem dominanten Term eine Einheit ist.
\end{lem}
\begin{proof}
...
\def\myT{\cD_{\Cftl}}
\def\myU{\cD_{\Cful}}
Sei $P\in\cD_\Cftl$ und $\cM:=\myT/\myT\cdot P$.
Wir wollen zeigen, dass
\begin{align*}
\rho^*\cM &\overset{!}{\cong}\myU/\myU\cdot Q 
\end{align*}
für $Q=P(\rho(u),\rho'(u)^{-1}\partial_u)$ gilt.
...
ist
\begin{itemize}
\item \textbf{exact}, weil $\cM\cong\myT\Big/\myT\cdot P=\coker(\_\cdot P)$
und
\item \textbf{flach}, da es Moduln über Körper, also Vektorräume, sind.
%TODO: quelle? falsch?
\end{itemize}
Deshalb ist auch, nach anwenden von $\Cful\otimes_\Cftl\_$, die Sequenz
...
Deshalb ist 
\begin{align*}
\rho^*\cM &\cong \coker(\id\otimes\_\cdot P)
  & \mbox{(weil exact)}\\
&\cong \Cful\otimes_{\Cftl}\myT \Big/(\id\otimes\_\!\cdot\!  P)\Big(
  \Cful\otimes_{\Cftl}\myT \Big) 
  & \mbox{(nach def. von $\coker$)}
\end{align*}
Also mit $\Phi$ wie in Lemma \ref{lem:pull-back-hilfslemma1} und
$Q(u,\partial_u):=P(\rho(u),\rho'(u)^{-1}\partial_u)$
...
Sequenz fortsetzen (weil $\_\!\cdot\! Q$ injektiv)
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
  \matrix (m) [
    matrix of math nodes
...
\end{comment}
Nun wollen wir mittels Diagrammjagd einen Isomorphismus
$\phi:\rho^*\cM\rightarrow \myU\Big/\myU\cdot Q$ finden. Sei dazu
$x\in\rho^*\cM$
\begin{comment}
leider kann man hier NICHT das (kurzem) Fünfer-Lemma anweinden\\
und damit gilt dann nach dem (kurzem) Fünfer-Lemma
\begin{align*}
\rho^*\cM &\overset{\phi}{\cong} \myU/\myU\cdot Q \\
          &= \myU\Big/\myU\cdot P(\rho(u),\rho'(u)^{-1}\partial_u) \,, \\
\end{align*}
falls es einen Morphismus $\phi$ überhaupt gibt.
\\
\\
kurzes 5er-lemma sagt nur (für den mittleren) das isomorph, falls oben und
unten exact, rechts und links isomorphisman, und ein möglicher morphismus für
Die mitte bereits existent. Muss auch alles Kommutieren.
\end{comment}
\begin{comment}
\begin{itemize}
