  ...
  \begin{defn}[Meromorpher Zusammenhang über $k$]
    ...
    \begin{itemize}
      ...
      \item einer $\C$-linearen Abbildung $\partial:\cM_K\rightarrow \cM_k$,
        ...
        erfüllen soll.
    \end{itemize}
    Falls $k=K$ reden wir einfach nur von einem \emph{Meromorphen Zusammenhang}
    und falls $k=\hat K$ reden wir von einem \emph{formalen Meromorphen
    Zusammenhang}.
  \end{defn}
  \begin{bem}
    Sei $\cM_K$ ein Meromorphen Zusammenhang, so erhalte $\cM_{\hat K}$ durch:
    ??? Tensor?
    % was bleibt erhalten?
  \end{bem}
\end{comment}
...
\begin{defn}[push-forward]
  ...
  \begin{itemize}
    \item der $\C(\!(t)\!)$-VR $\rho_*\cN$ ist der $\C$-VR $\cN$ mit der
      $\C(\!(t)\!)$ Struktur durch $f(t)\cdot m:=f(\rho(t))m$
      % für alle m aus ???
