...
\paragraph{Systeme von ODEs}
%TODO: \tilde cO --> \tilde \cO
\begin{comment}
\cite[Chap 5.1.1]{hotta2007d} %abgeschrieben
\end{comment}
Für eine Matrix $A(x)=(a_{ij}(x))_{ij}\in M(n\times n,K)$ definieren wir das
\emph{System von gewöhnlichen Differentialgleichungen (kurz ODEs)} als
\begin{equation}
...
von unbekannten Funktionen. Wir werden (\ref{eq:KlassischesODE}) immer in
einer Umgebung um $x=0\in\C$ betrachten. Als Lösungen von
(\ref{eq:KlassischesODE}) betrachten wir Keime von holomorphen (aber
möglicherweise mehrdeutigen) %mehrwertigen
...
Wir sagen $v(x)=\,^t(v_1(x),\dots,v_n(x))$ ist eine Lösung von
(\ref{eq:KlassischesODE}), falls $ v_i\in\tilde \cO$ für alle
...
Zusammenhänge über $k$. Eine $k$-lineare Abbildung $\phi:\cM\rightarrow\cN$ ist
ein Morphismus von meromorphen Zusammenhängen, falls sie
$\phi\circ\partial_\cM=\phi\circ\partial_\cN$ erfüllt. In diesem Fall schreiben
wir auch $\phi:(\cM,\partial_\cM)\rightarrow(\cN,\partial_\cN)$.
\end{defn}
\begin{comment}
...
\begin{comment}
\begin{defn}
Wir erhalten damit die Kategorie dier meromorphen Zusammenhänge über $k$
mit
\begin{align*}
...
von $K$ verzichtet, sofern klar ist, welches $K$ gemeint ist.
\begin{comment}
\cite[Rem 5.1.2.]{hotta2007d}
...
Matrix $A(x)=(a_{ij}(x))_{i,j\in\{1,\dots,n\}}\in M(n\times n,K)$ definiert
durch
...
\iffalse
  ...
  \end{align*}
\fi
Genauer ausgeführt wird dies beispielsweise in \cite[Sec 5.1]{hotta2007d}.
...
Linearformen verwedet, dieser ist hier aber bereits für $\Ckt$ reserviert.
...
\end{tikzpicture}
\end{center}
und damit folgt, wegen Isomorphie der Cokerne, die Behauptung.
\end{proof}
...
Der \emph{push-forward} oder das \emph{direktes Bil} $\rho_+\cN_{\hat L}$ von
$\cN_{\hat L}$ ist
\begin{itemize}
