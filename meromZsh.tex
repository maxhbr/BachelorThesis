...
\begin{defn}
Definiere
\[ \hat\cD[u^{-1}]=\hat K<\partial_u>=:\cD_{\hat K} \]
wobei $\hat K=\Cfu[u^{-1}]$
\end{defn}
...
\begin{lem}
Ist $\cM=\cD_{\Cftl}/\cD_{\Cftl}\cdot P(t,\partial_t)$ für ein $P(t,\partial_t)\in\cD_{\Cftl}$ so ist
...
\def\myT{\cD_{\Cftl}}
\def\myU{\cD_{\Cful}}
Wie erhält man den pull-back Zusammenhang bzw. wie ist er berechenbar? Sei
...
\end{tikzpicture}
\end{center}
und mit $Q:=\rho^+P(t,\partial_t):=P(\rho(u),\rho'(u)^{-1}\partial_u)$ ergibt
sich übersichtlich die Situation
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
\matrix (m) [
  matrix of math nodes
...
\end{tikzpicture}
\end{center}
wobei die untere und die obere Zeile jeweils Exacte Sequenzen sind.
Und damit gilt dann
\begin{align*}
