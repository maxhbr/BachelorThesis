...
Zusammenhänge. Eine $K$-lineare Abbildung $\phi:\cM\rightarrow\cN$ ist ein
...
Linearformen verweden, dieser ist hier aber bereits für $\Ckt$ reserviert.
...
\cD_{\hat L }$ als $\cD_{\hat L}$-Moduln, mittels
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
\matrix (m) [
  matrix of math nodes
...
\end{lem}
\begin{proof}
Prüfe zunächst die Injectivität. Sei $f(t)\otimes m(x,\partial_x)$ so, dass
\begin{align*}
0 &= \Phi(f(t)\otimes m(x,\partial_x))
\\&= f(t)m(\rho(t),\rho'(t)^{-1}\partial_t)
\end{align*}
und, da hier alles Nullteilerfrei ist, ist die Bedingung äquivalent zur
folgenden
\begin{align*}
\Leftrightarrow && 0&=f(t) &\text{oder}&& 0&=m(\rho(t),\rho'(t)^{-1}\partial_t)
\\\Leftrightarrow& \dots
\end{align*}
...
\\&=\sum_ka_k(t)\underset{=1}{\underbrace{\rho'(t)\rho'(t)^{-1}}}\partial_t^k
\\&=\rho'(t)\sum_ka_k(t)\rho'(t)^{-1}\partial_t^k
\\&= \dots
\end{align*}
...
Mit $\Phi$ wie in \label{lem:pull-back-hilfslemma1pre} gilt sogar
$\rho^*\cD_{\hat K }=\hat L\otimes_{\hat K}\cD_{\hat K } \cong \cD_{\hat L }$
als Meromorphe Zusammenhänge.
\end{lem}
\begin{proof}
Wir wollen noch zeigen, dass $\partial_\otimes\circ\Phi = \Phi \circ
\partial_t$ ist. Betrachte dazu das Diagram
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
\matrix (m) [
  matrix of math nodes
...
\end{tikzpicture}
\end{center}
und für ein $1\otimes Q(x,\partial_x)\in\hat L\otimes_{\hat K}\cD_{\hat K }$
folgt dann
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
\matrix (m) [
...
1\otimes Q(x,\partial_x) &  & \rho'(t)\otimes \partial_xQ(x,\partial_x)\\
& & \underset{=1}{\underbrace{\rho'(t)\cdot \rho'(t)^{-1}}}
  \partial_tQ(\rho(t),\rho'(t)^{-q}\partial_t)\\
Q(\rho(t),\rho'(t)^{-1}\partial_t) & 
  \partial_tQ(\rho(t),\rho'(t)^{-q}\partial_t)\\
};
%\path (m-1-1) edge[white] node{$\%$} (m-2-2);
\path[|->,font=\scriptsize,>=angle 90]
(m-1-1) edge node[right]{$\Phi$} (m-3-1)
(m-1-1) edge node[above]{$\partial_\otimes$} (m-1-3)
(m-3-1) edge node[above]{$\partial_t$} (m-3-2)
(m-1-3) edge node[right]{$\Phi$} (m-2-3)
;
\end{tikzpicture}
\end{center}
\begin{comment} TODO \end{comment}
