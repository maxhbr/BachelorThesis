...
exisitiert eine (bis auf Permutation) eindutige Zerlegung 
\[
...
%
Sei $(\rho:\C\rightarrow \C , u\mapsto t:=\rho(u))\in u\Cfu$ mit Bewertung
...
Der \emph{pull-back} oder das \emph{Inverses Bild} $\rho^{+}\cM$ von $\cM$ ist
der Vektorraum $\rho^{*}\cM=\C(\!(u)\!)\otimes_{\C(\!(t)\!)}\cM$ mit dem
\emph{pull-back Zusammenhang} $\rho^*\nabla$ definiert durch
...
für Elementartensoren $1\otimes m\in \Cful\otimes_{\Cftl}\cD_{\Cftl}$
\begin{align*}
...
Weil $\Cful$ \textbf{flach} ist, da  Körper, ist auch, nach anwenden des
exacten Funktors $\Cful\otimes_\Cftl\_$, die Sequenz
...
$\cM_{\hat K}$ und $\rho:u\mapsto t:=u^p$, dann gilt für $\cP(\rho^*\cM_{\hat
K})=\{\lambda_1',\dots,\lambda_r'\}$, dass $\lambda_n'=p\cdot\lambda_n$.
\end{lem}
\begin{proof}
Sei $\cM_{\hat K}=\cD_{\hat K}\slash \cD_{\hat K}\cdot P$ mit $P=\sum
a_i(t)\partial_t^i$, dann ist
$\rho^*\cM_{\hat K}=\cD_{\hat K}\slash \cD_{\hat K}\cdot P'$ mit 
\begin{align*}
P' &= \sum a_i(\rho(t))(\rho'(t)^{-1}\partial_u)\\
   &=\sum a_i(u^p)(p\cdot u^{-(p-1)}\partial_u)
\end{align*}
\end{proof}
%
