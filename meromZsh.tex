...
Für $P(\rho(t),\rho'(t)^{-1}\partial_t)$ werden wir auch $\rho^*P$schreiben.
\end{thm}
\begin{comment}
...
Zusammenhängen ist. Betrachte dazu das Diagram
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
\matrix (m) [
  matrix of math nodes
...
\end{tikzpicture}
\end{center}
und für einene Elementartensor $f(t)\otimes Q(x,\partial_x)\in\hat
L\otimes_{\hat K}\cD_{\hat K }$ folgt dann
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
\matrix (m) [
  matrix of math nodes
...
\end{tikzpicture}
\end{center}
also kommutiert das Diagram mit $\alpha=\_\!\cdot\!
P(\rho(t),\rho'(t)^{-1}\partial_t)$.
\end{proof}
...
ist exact, weil $\cM_{\hat K } \cong\cD_{\hat K }\Big/\cD_{\hat K
