...
Das Newton-Polygon hängt nur von dem assoziierten Meromorphen Zusammenhang ab.
\end{lem}
...
\]
ist. Dies stellt eine Normierung des Newton Polygons dar.
\begin{lem}
...
\end{tikzpicture}
\end{center}
Nun konstruieren wir mittels Diagrammjagd den Isomorphismus
%\[
%\phi:\rho^*\cM = \hat L\otimes_{\hat K}\big(\cD_{\hat K}\big\slash\cD_{\hat K}
%\cdot P\big)\rightarrow \cD_{\hat K }\big/\cD_{\hat K }\cdot Q \,,
%\]
\begin{center}
\begin{tikzpicture} [scale=3.3, descr/.style={fill=white,inner sep=2.5pt} ]
  \matrix (m) [
...
\hat L\otimes_{\hat K}\big(\cD_{\hat K}\big\slash\cD_{\hat K} \cdot P\big)
  & \cD_{\hat K }\big/\cD_{\hat K }\cdot Q \,,\\
  };
  %TODO: Pfeile
  \path[->,font=\scriptsize,>=angle 90]
...
\end{tikzpicture}
\end{center}
so dass $\phi\circ(\id\otimes\pi_{\hat K})=\pi_{\hat L}\circ\Phi$, also das
obige Diagram kommutiert.
Sei $x_0\in\hat L\otimes_{\hat K}\cM_{\hat K}$, so existiert, da
$\id\otimes\pi_{\hat K}$ surjektiv ist, ein $x_1\in\hat L\otimes_{\hat
K}\cD_{\hat K}$ mit $\id\otimes\pi_{\hat K}(x_1)=x_0$. So setzen wir
$\phi(x_0)=\pi_{\hat L}\circ\Phi (x_1)$ als das Bild von $x_0$ unter $\phi$.
Nun konstruieren wir noch den Umkehrmorphismus $\psi$ zu $\phi$.
Sei $y_0\in\cD_{\hat L }\Big/\cD_{\hat L }\cdot Q$ so existiert ein
$y_1\in\cD_{\hat L }$ mit $\pi_{\hat L}(y_1)=y_0$. Wir setzen
$\id\otimes\pi_{\hat K}\circ\Phi^{-1}(y_1)=\psi(y_0)$.
%TODO: das folgende
\[
\phi\circ\psi=\dots=\id
\]
\[
\psi\circ\phi=\dots=\id
\]
...
\begin{comment}
\begin{bem}[Nach Robert] % von Robert
...
\fbox{
  ...
  \def\myPath{-- (2,-1)}
  \myPlotFunction{\myPoints}{\myPath}{2}{-1}{-1}{
...
&= \rho_+\cN_{\hat L}\otimes_{\hat K}\cM_{\hat K}
  & \mbox{(?)}
\end{align*}                               sudo rm /etc/asound.conf
      alsactl restore 0
...
\end{comment}
\begin{defn}[Fouriertransformation von Meromorphen Zusammenhang]
