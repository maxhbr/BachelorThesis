...
\begin{defn}
Definiere
\[ \hat\cD[x^{-1}]=\cD_\Cfx[x^{-1}]=\hat K<\partial_x>=:\cD_{\hat K} \]
wobei $\hat K=\Cfu[x^{-1}]$
\end{defn}
\begin{defn}[Meromorpher Zusammenhang]
Ein \emph{Meromorpher Zusammenhang}
$(\cM_{\hat K},\partial)$ besteht aus folgenden Daten:
\begin{itemize}
\item $\cM_{\hat K}$, ein endlich dimensionaler $\hat K$-Vektor Raum
\item einer $\C$-linearen Abbildung $\partial:\cM_{\hat K}\rightarrow
\cM_{\hat K}$,
genannt \emph{Derivation} oder \emph{Zusammenhang}, welche für alle $f\in \hat
K$
und $u\in \cM_{\hat K}$ die
\emph{Leibnitzregel}
\begin{equation}\label{eq:Leibnitzregel}
\partial(fu)=f'u+f\partial u
\end{equation}
...
\cD_{\hat K}$ mit $\cM=\cD_{\hat K}/\cD_{\hat K}\cdot P$.
\end{bem}
...
t:=\rho(u))\in u\Cfu$ mit Bewertung
...
$ \slopes(\rho^+P)=\{1\} $ (siehe Abbildung \ref{fig:Pull-Back2}) und damit
