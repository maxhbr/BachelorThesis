%!TeX root = main.tex
\chapter{Operationen auf meromorphen Zusammenhängen} \label{chap:operationen}
...
Hier zunächst zwei Rechenregeln die für jedes Tensorprodukt gelten:
\begin{align}
...
\end{align}
Sei $f:M'\rightarrow M$ eine Abbildung, so gilt allgemein
\begin{align}
...
Sei\linebreak $n\otimes n\in \cM\otimes_K\cN$.
Durch Setzen von
...
$\cM_{\hat K}=\cD_{\hat K}/\cD_{\hat K}\cdot P(x,\partial_x)$ für ein\linebreak
...
\end{align*}
\end{comment}
Prüfe zunächst die Injektivität. Sei 
$\sum_{k=0}^{p-1}t^k\otimes Q_k(x,\partial_x)$ ein allgemeines Element,
welches
\begin{align*}
0 &= \Phi\Big(\sum_{k=0}^{p-1}t^k\otimes Q_k(x,\partial_x)\Big)
\end{align*}
erfüllt, also in $\ker(\Phi)$ liegt. Es ist
\begin{align*}
\Phi\Big(\sum_{k=0}^{p-1}t^k\otimes Q_k(x,\partial_x)\Big)
  &= \sum_{k=0}^{p-1}t^k Q_k(\rho(t),\rho'(t)^{-1}\partial_t) =0
\end{align*}
eine eindeutige Zerlegung des Bildes, wobei jede Potenz von $t$ in nur genau
einem Summanden vorkommt. Also ist diese Summe gleich $0$, genau dann, wenn
jeder einzelne Summand verschwindet. Also
\begin{align*}
\Phi\Big(\sum_{k=0}^{p-1}t^k\otimes Q_k(x,\partial_x)\Big)&=0
&\Leftrightarrow&&
\Phi\Big(t^k\otimes Q_k(x,\partial_x)\Big)=0 
\qquad\forall k\in\{0,\dots,p-1\}
\end{align*}
und, da hier alles nullteilerfrei ist und da $t^k\neq0\in \hat L$ $\forall k$,
ist die Bedingung äquivalent zu
\begin{align*}
\Leftrightarrow\qquad 0&=Q_k(\rho(t),\rho'(t)^{-1}\partial_t)
  \qquad\forall k\in\{0,\dots,p-1\}
\\\Leftrightarrow\qquad 0&=Q_k(x,\partial_x)
  \qquad\forall k\in\{0,\dots,p-1\}
\\\Leftrightarrow\qquad 0&=t^k\otimes Q_k(x,\partial_x) 
  \qquad\forall k\in\{0,\dots,p-1\} \,.
\end{align*}
Damit folgt, dass $\sum_{k=0}^{p-1}t^k\otimes Q_k(x,\partial_x)=0$, womit die
Injektivität gezeigt ist.
\begin{comment}
...
(\frac{1}{2t}\partial_t)^2
  &= \frac{1}{2t}\myubracket{\partial_t \frac{1}{2t}}\partial_t
\\&\!\!\overset{(\ref{eq:kommutator1})}{=}\!\! \frac{1}{2t}
  \myobracket{(-\frac{1}{2t^2} + \frac{1}{2t}\partial_t)}\partial_t
...
$ \cP(\rho^*P)=\{1\} $ (siehe Abbildung \ref{fig:Pull-Back2}) und somit ist der
Pullback gegeben durch $\rho^*\cM_{\hat K}=\cD_{\hat L}/\cD_{\hat L}
\cdot(\frac{1}{4}t^4\partial_t^2-\frac{9}{4}t^3\partial_t-1)$.
\begin{figure}[htbp]
  \begin{minipage}[hbt]{0,49\textwidth}
  \begin{center}
    \begin{tikzpicture}[scale=2.1,descr/.style={fill=white,inner sep=2.5pt}]
...
\matrix (m) [
  ...
  \cdot:\hat{K}\times\cN_{\hat{L}}  & \cN_{\hat{L}}\phantom{\hat{K}} \\
  (f(x),m) & f(x)\cdot m:=f(\rho(t))m\,. \\
};
%TODO: Pfeile
\path[->,font=\scriptsize,>=angle 90]
(m-1-1) edge (m-1-2)
...
\end{equation}
\end{thm}
\begin{proof} Mit einfachen Umformungen gilt:
\begin{align*}
...
$\cM_{\hat K}$.
Wir wollen den Zusammenhang bei Unendlich betrachten und dazu den Übergang
$x\rightsquigarrow z^{-1}$ durchführen. 
...
Polygons sein können, $(1,\textcolor{blue}{-1})$ und
$(0,\textcolor{red}{-(p+1)})$ sind. Da einer der relevanten
Punkte auf der vertikalen Achse liegt, kann es insgesamt nur einen Slope
$\Lambda$ geben, welcher sich wie folgt berechnet:
\begin{align*}
\Lambda&=\max\left\{0,\frac{
  \textcolor{blue}{-1}-(\textcolor{red}{-(p+1)})}{1}\right\}
=\max\{0,p\}
=\begin{cases}
  p &\text{, wenn }p\geq0
\\0 &\text{, wenn }p<0
...
Da der Grad von $P$ gleich $q$ ist, ist
\iffalse
  nach Lemma \ref{lem:twistRechenregel}
  auch $Q$ von Grad $q$ und somit
\fi
auch $\dim_{\hat K}\cN= q$.
...
\[
\cM_{\hat K}\otimes_{\hat K}\sE^{\phi}\cong\cD_{\hat K}/\cD_{\hat K}\cdot
Q(x,\partial_x)
\]
...
\pagebreak
Somit erhalten wir, wenn wir diese Transformation auf
$\left[\sum_{j=0}^{p-1}t^{j-1}\psi_j(t^p)V^j\right]$ anwenden:
\begin{align*}
...
\end{align*}
da $\phi'(t)=pt^{p-1}$.
Damit wissen wir bereits, dass im folgendem Diagramm
