...
\\\Rightarrow v_{n+2} &= - \frac{1}{u_{-2}} 
  ((n+1)u_{n+1} + \sum_{k=0}^{n+2} v_{k-1}u_{n-k+1})
\end{align*}
also nach passendem Indexshift folgt
\begin{align*}
...
i\Big((n-1)\frac{u_{n-1}}{2\sqrt{2a}} 
  ...
  & \forall n \geq 0
\end{align}
die Koeffizienten vollständig bestimmt.

Somit ergeben sich für $n=0$ die Koeffizienten
\begin{align*}
v_{0} &= - \frac{1}{2i\sqrt{2a}}
  ((-1)u_{-1} + \sum_{k=0}^{0} v_{k-1}u_{-k-1})
\\&= - \frac{1}{2i\sqrt{2a}} (\frac{3}{2} + v_{-1}u_{-1})
\\&= - \frac{1}{2i\sqrt{2a}} (\frac{3}{2} - \frac{3}{4})
\\&= - \frac{3}{8i\sqrt{2a}} = -u_{0}
\end{align*}
Für $n>0$ wollen wir die Formel noch weiter vereinfachen:
\begin{align*}
v_{n} &= - \frac{1}{2i\sqrt{2a}}
  ((n-1)u_{n-1} + \underbracket{\sum_{k=0}^{n} v_{k-1}u_{n-k-1}})
\\&= - \frac{1}{2i\sqrt{2a}}
  ((n-1)u_{n-1} + \overbracket{v_{-1}u_{n-1} + \sum_{k=1}^{n-1}
  v_{k-1}u_{n-k-1} + v_{n-1}u_{-1}})
\\&\overset{(\ref{eq:bedingung1c})}{=} - \frac{1}{2i\sqrt{2a}}
  (-(n-1)v_{n-1} + v_{-1}(-v_{n-1}) - \sum_{k=1}^{n-1}
  v_{k-1}v_{n-k-1} + v_{n-1}u_{-1})
\\&= - \frac{1}{2i\sqrt{2a}} (-(n-1)v_{n-1} -\frac{1}{2}v_{n-1} - \sum_{k=1}^{n-1}
  v_{k-1}v_{n-k-1} - \frac{3}{2} v_{n-1})
\\&= \frac{1}{2i\sqrt{2a}} ((n-1+\frac{1}{2}+\frac{3}{2})v_{n-1} 
  + \sum_{k=1}^{n-1} v_{k-1}v_{n-k-1})
\\&= \frac{1}{2i\sqrt{2a}} ((n+1)v_{n-1} + \sum_{k=1}^{n-1} v_{k-1}v_{n-k-1})
\\&\overset{(\ref{eq:bedingung1a})}{=}
  \frac{1}{2i\sqrt{2a}} ((n+1)v_{n-1} + \sum_{k=1}^{n-1} v_{k-1}v_{n-k-1})
\\&\overset{(\ref{eq:bedingung1c})}{=}-u_{n}
\end{align*}
...
\subsection{Berechnung der Faktoren}
