...
\iffalse
  \begin{try}
  Um eine nichttriviale Zerlegung zu bekommen, müssen die $Q_i$ zumindest den
  ...
  \\&\qquad + \overbracket{\sum_{n=-\max((l_1+l_2),l_2+1)}^\infty
  ...
  \end{try}
  \begin{try}
  Hierfür betrachten wir das verschobene Minimalpolynom $t^{-4}P'=\partial_t^2 -
  (t^{-1} - 2i\sqrt{2a}t^{-2})\partial_t - 3 i\sqrt{2a}t^{-3}$
  \end{try}
  \newpage\begin{try}
  Hierfür betrachten wir das verschobene Minimalpolynom 
  \begin{align*}
  t^{-1}P'&=t^3\partial_t^2 + (t^{2} - 2i\sqrt{2a}t^{1})\partial_t - 3 i\sqrt{2a}
    & \in \cD_\Cft \subset \cD_{\hat L}
  \end{align*}
  \begin{figure}[H]
  \begin{center}
  \fbox{
  ...
  \end{center}
  \end{figure}
  und wir suchen $Q_1$ und $Q_2\in \cD_{\hat L}$ so dass, $t^{-1}P'=Q_1\cdot
  Q_2$. Da der $\partial_t$-Grad von $t^{-1}P'$ genau $2$ ist, müssen die $Q_i$
  jeweils den Grad $1$ haben, um eine nichttriviale Zerlegung zu bekommen.
  \begin{comment}
  Da $t^{-1}P'\in\cD_\Cft$ ist, werden auch die $Q_i$ in $\cD_\Cft$ sein.
  \end{comment}
  \begin{beo} \label{beo:paarweise-verschieben}
  %TODO: move to theorie-teil??
  Ist $Q_1$ und $Q_2$ so ein Paar, dann ist für $\sigma\in \hat K$ das Paar
  $Q_1':=Q_1\cdot \sigma^{-1}$ und $Q_2':=\sigma\cdot Q_2$ ebenfalls eine
  Zerlegung.
  \end{beo}
  \begin{proof}
  TODO / klar
  ...
  den Faktor vor den $\partial_t$ in $Q_2$ frei wählen können. Setze 
  also setze allgemein
  \begin{align*}
  ...
  \\&\overset{!}{=} t^3\partial_t^2 + (t^{2} - 2i\sqrt{2a}t^{1})\partial_t 
    - 3 i\sqrt{2a}
  \end{align*}
  Somit können wir $\bar v(t)=t^3$ setzen.
  \end{try}
\fi
Hierfür betrachten wir das verschobene Minimalpolynom 
\begin{align*}
