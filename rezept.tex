...
Nun wollen wir in diesem Unterabschnitt noch das Konvergenzverhalten
betrachten. Nach allgemeineren Resultaten, welche hier aber nicht erläuterten
werden, ist es klar, dass echt formale Potenzreihen nötig sind.
...
Die zwei geprüften Konvergenzkriterien weisen beide darauf hin, dass die
Polynome $v(t)$ und $u(t)$ divergieren, also in $\hat L\backslash L$ liegen.
Ein nächstes Ziel wäre es, eines der Konvergenzkriterien konkret anzuwenden,
um den exacten Konvergenzradius Analytisch zu bestimmen.
\begin{comment}
%TODO: bekannt? woher?
...
Ganz unabhängig vom Konvergenzverhalten, welches in Abschnitt
\ref{sec:konvergenzDerPotReihen} betrachtet wurde, kann das Ergebnis aus
\ref{sec:LT-speziell} genutzt werden, um die Stokes Struktur von
$\rho^+\cM_\phi$ zu berechnen.

\begin{comment}
\begin{itemize}
\item asymptotische Analysis
\item Stokes Struktur
\item nicht wirklich möglich, Aussagen zu treffen
\end{itemize}
\end{comment}
