...
\end{center}
\end{figure}
Berechne nun den pull-back mit $\rho:t\mapsto x:=-2t^2$
\begin{align*}
\rho^*P_{\phi_1}(x,\partial_x)
  &=t^{3}\partial_t(\frac{1}{2}t\partial_t-1)+a\\
  &=\frac{1}{2}t^{3}\underbracket{\partial_tt}\partial_t-t^{3}\partial_t+a\\
  &=\frac{1}{2}t^{3}\overbracket{(t\partial_t+1)}\partial_t-t^{3}\partial_t+a\\
  &=\frac{1}{2}t^{4}\partial_t^2+\frac{1}{2}t^{3}\partial_t-t^{3}\partial_t+a\\
  &=\frac{1}{2}t^{4}\partial_t^2-\frac{1}{2}t^{3}\partial_t+a
\end{align*}
\iffalse
  \begin{comment}
  \begin{align*}
...
\begin{align*}
\sigma_L(\rho^*P_{\phi_1}) &= \frac{1}{2}\theta^2 + a
\end{align*}
schreiben, welches wir als nächstes faktorisieren
\begin{align*}
\sigma_L(\rho^*P_{\phi_1}) &= \frac{1}{2}\theta^2+a\\
  &=\frac{1}{2}(\theta^2-4a)\\
  &=\frac{1}{2}(\theta-\underset{=:\beta_0}{\underbrace{2\sqrt{a}}})
    (\theta+2\sqrt{a})
\end{align*}
Dies geht, weil $\hat K[\xi]$ kommutativ ist.
Setze $\psi(x):=(\beta_0/(\lambda_0+1))x^{\lambda_0+1}=\sqrt{a}x^2$ und
betrachte $\cN:=\rho^+\cM_{\phi_1}\otimes\sE_{\hat K}^\psi$ mit dem zyklischem
Vektor $e\otimes 1$, wobei $e$ ein zyklischer Vektor von $\rho^+\cM$.
Es ist
\begin{align*}
\partial_t^2(e\otimes 1) 
  ...
  &= (t^{-1}\partial_t-2t^{-4}a)e\otimes 1
  + \underbracket{2qt^{q-1}(\partial_t e)\otimes 1}
  + (q(q-2)t^{q-2} + (qt^{q-1})^2)(e\otimes 1)\\
  &\begin{aligned}
    & = \overbracket{2qt^{q-1}\partial_t (e\otimes 1) -
      \underbracket{2qt^{q-1}(\psi'(t)(e\otimes 1))}}
      +t^{-1}\partial_t(e\otimes 1) \\
    &\qquad + (-2t^{-4}a + q(q-2)t^{q-2} 
      + (qt^{q-1})^2)(e\otimes 1)\\
  \end{aligned}\\
  &\begin{aligned}
    & = 2qt^{q-1}\partial_t (e\otimes 1) - \overbracket{2(qt^{q-1})^2(e\otimes
    1)} +t^{-1}\partial_t(e\otimes 1) \\
    &\qquad + (-2t^{-4}a + q(q-2)t^{q-2} + (qt^{q-1})^2)(e\otimes 1)\\
  \end{aligned}\\
  & = (2qt^{q-1}+t^{-1})\partial_t (e\otimes 1) + (-2t^{-4}a + q(q-2)t^{q-2} -
    (qt^{q-1})^2)(e\otimes 1)\\
  & = \Big((2qt^{q-1}+t^{-1})\partial_t - 2t^{-4}a + q(q-2)t^{q-2} -
    (qt^{q-1})^2\Big)(e\otimes 1)\\
\end{align*}
\begin{comment}
TODO: verbessern
\end{comment}
Also ist 
\[
\cN=\hat\cD/\hat\cD\cdot ( \partial_t^2-((2qt^{q-1}+t^{-1})\partial_t +
(-2t^{-4}a + q(q-2)t^{q-2} - (qt^{q-1})^2)))
\]
\begin{comment}
\begin{align*}
...
\end{align*}
\end{comment}
und Aufgrund der linearen Abhängigkeit existieren $a_0$ und $a_1$, so dass
\[
\partial_x^2 (e\otimes 1) = a_0 e\otimes 1 + a_1\partial_x (e\otimes 1)
\]
