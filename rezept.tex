...
Hier hilft uns aber ein Trick, nach Korollar \ref{cor:zurücktwisten} gilt
$
\rho^+\cM_{\phi}
...
eine Konstante ist.
\end{comment}
Sei $\beta$  eine der Nullstellen von $\sigma_L(\rho^*P_{\phi})(\theta)$.
...
Wir suchen nun ein $\psi$, so dass der Twist $\rho^+\cM_{\phi}\otimes\sE^\psi$
mehr als einen slope hat, damit wir entlang der slopes zerlegen können.
Berechne dazu die \emph{Determinanten Gleichung}
...
\fi
Mit dem Lemma vom Zyklischen Vektor, bzw. dem Korollar
\ref{cor:Zyklischer-Vektor}, wollen wir nun ein Minimalpolynom zu $\cN$
berechnen\footnote{
...
\\&\overbox{=}{Cauchy Produkt} t^{-3} \myobracket{\sum_{n=0}^\infty
...
Nun zum Konvergenzverhalten. Es ist klar, dass
\begin{align*}
