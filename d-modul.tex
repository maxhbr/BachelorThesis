...
Falls
\begin{itemize}
\item $k=\C[x]$ schreibe $A_1(\C):=\cD_{\C[x]}$
\item $k=\C\{x\}$ schreibe $\cD:=\cD_{\C\{u\}}$
\item $k=\Cfx$ schreibe $\hat\cD:=\cD_{\Cfu}$
\item $k=K\overset{\mbox{def}}{=}\C(\{x\}):=\C\{x\}[x^{-1}]$ schreibe $\cD_K$
\item $k=\hat K\overset{\mbox{def}}{=}\Cfxl:=\Cfx[x^{-1}]$ schreibe $\cD_{\hat
K}$
\end{itemize}
\end{defn}
\end{comment}
